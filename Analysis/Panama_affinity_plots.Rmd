---
title: "Panama Fungi Affinity Plots"
author: "Natalie Winans, Demi Glidden, Susan Collins"
date: "2/16/2020"
...
knitr::opts_chunk$set(echo = TRUE,
                      ...
                      warning = FALSE,
                      fig.pos = "center",
                      fig.path = "~/UO/Fall_20/group-projects-panama-microbes-project/Analysis/Plots/")
...
setwd("~/Desktop/UO/Fall_20/group-projects-panama-microbes-project/affinity_plots")
```
...
```{r affinity assignment loops, cache = TRUE, dependson = c("reshape data")}
...
Threshold for strong effect of resin phosphate. Low affinity defined as effect size < -0.8 (blue points and lines). High affinity defined as effect size > 0.8 (red points and lines). The proportion of ASVs with low resin P affinity and that of ASVs with high resin P affinity is equal at 0.92 mg/kg. Axes are scaled: x on log scale, y on square root scale.

### MAP affinity threshold

```{r MAP affinity plot, fig.cap=""}
############## MAP ##############
...
Threshold for strong effect of mean annual precipitation (MAP). Low affinity defined as effect size < -0.8 (blue points and lines). High affinity defined as effect size > 0.8 (red points and lines). The proportion of ASVs with low MAP affinity and that of ASVs with high MAP affinity is equal at 2567 cm. Y axis is square root scaled.

### DON affinity threshold

```{r DON affinity plot, fig.cap=""}
############## DON ##############
...
Threshold for strong effect of dissolved organic nitrogen (DON). Low affinity defined as effect size < -0.8 (blue points and lines). High affinity defined as effect size > 0.8 (red points and lines). The proportion of ASVs with low DON affinity and that of ASVs with high DON affinity is equal at 15.76 mg/kg. Axes are square root scaled.


### Phosphorus (Resin P) affinity group composition

```{r P affinity phylum, fig.width=6}
...
```{r P affinity class, fig.width=8}
# Class
...
```{r MAP affinity phylum, fig.width=6}
...
```{r MAP affinity class, fig.width=9}
# Class
...
```{r N affinity phylum, fig.width=6}
...
```{r N affinity class, fig.width=9}
# Class
